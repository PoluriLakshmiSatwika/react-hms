...
const DoctorDashboard = () => {
  ...
  const NurseAssignment = ({ appointment, nurses, onAssign }) => {
    const [selectedNurses, setSelectedNurses] = useState(appointment.assignedNurse ? [appointment.assignedNurse] : []);
        ...
        appointments.map(app => (
          <div key={app._id} className="appointment-card">
            <div className="appointment-header" onClick={() => togglePanel(app._id)}>
              <div>
                <h3>{app.patientId?.fullName || "Unknown Patient"}</h3>
                <p>{app.slotTime} on {new Date(app.appointmentDate).toLocaleDateString()}</p>
                <p><b>Assigned:</b> {app.assignedNurse ? getNurseNames([app.assignedNurse]) : "None assigned"}</p>
              </div>
