...
/* =================== DUMMY DATA (used when API fails or empty) =================== */
...
const NurseAssignment = ({ appointment, nurses, onAssign }) => {
  ...
  return (
    <div className="assignment-box">
      ...
      <button className="save-btn" onClick={() => onAssign(appointment._id, selectedNurses)}>
...
const DoctorDashboard = () => {
  ...
  // ✅ Fetch Appointments (use dummy if no data)
  const fetchAppointments = async () => {
    try {
      ...
      if (res.data.success && res.data.data.length > 0) {
        ...
        console.warn("⚠ No appointments → using dummy");
        setAppointments(dummyAppointments);
      }
    } catch {
      console.warn("❌ API error → using dummy appointments");
      setAppointments(dummyAppointments);
    }
  };


  // ✅ Fetch Nurses (use dummy if no data)
  const fetchNurses = async () => {
    try {
      ...
      if (res.data.success && res.data.data.length > 0) {
        ...
        console.warn("⚠ No nurses → using dummy");
        setNurses(dummyNurses);
      }
    } catch {
      console.warn("❌ API error → using dummy nurses");
      setNurses(dummyNurses);
    }
  };
  ...
  const handleNurseAssignment = async (appointmentId, nurseIds) => {
    try {
      const res = await API.put("/api/doctor/assign-nurses", {
        appointmentId,
        nurseIds
      });
        ...
        appointments.map(app => (
          <div key={app._id} className="appointment-card">
            ...
            <div>
              ...
              <p>{app.slotTime} on {app.appointmentDate.split("T")[0]}</p>
